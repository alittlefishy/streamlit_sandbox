...
with st.expander("🔬 Study Design", expanded=True):
    ...
    with col2:
        tail = st.radio("Test Sidedness", ["Two-sided", "One-sided"], help="Two-sided tests detect any difference. One-sided tests detect effects in one direction.")
        alternative = "two-sided" if tail == "Two-sided" else "larger"

    # Only show margin if test type needs it
